...
import { validateEvaluationBundle } from "../contracts/semantic-validation.mjs";
...
const UNSAFE_DISPLAY_CHARACTER = /[\p{Cc}\p{Cf}\p{Zl}\p{Zp}]/u;
const MARKUP_DISPLAY_CHARACTER = /[#<>`*_~\[\]]/u;
const IMPERATIVE_DISPLAY_LANGUAGE = /\b(?:act\s+now|must|shall|will|immediately|authori[sz](?:e|ed|ation)?|approve[sd]?|execute[sd]?|transfer|command)\b/iu;
...
const evaluationRunSchema = JSON.parse(readFileSync(
  new URL("../contracts/schema/evaluation-run.schema.json", import.meta.url),
  "utf8",
));
const observationSchema = JSON.parse(readFileSync(
  new URL("../contracts/schema/predicate-observation.schema.json", import.meta.url),
  "utf8",
));
...
const evaluationAjv = new Ajv2020({ allErrors: true, strict: true });
addFormats(evaluationAjv);
const validateEvaluationRunSchema = evaluationAjv.compile(evaluationRunSchema);
const validateObservationSchema = evaluationAjv.compile(observationSchema);
...
function expressionRoles(expression, {
  polarity = "positive",
  required = true,
  role = "enabling",
} = {}) {
  if (!expression || typeof expression !== "object") return [];
  if (typeof expression.predicate_ref === "string") {
    return [{
      predicate_ref: expression.predicate_ref,
      polarity,
      required,
      role,
    }];
  ...
  if (Array.isArray(expression.all)) {
    return expression.all.flatMap((child) => expressionRoles(child, { polarity, required, role }));
  }
  if (Array.isArray(expression.any)) {
    return expression.any.flatMap((child) => expressionRoles(child, {
      polarity,
      required: false,
      role,
    }));
  ...
  if (expression.not) {
    return expressionRoles(expression.not, {
  ...
  if (expression.alternative_if) {
    return [
      ...expressionRoles(expression.alternative_if.condition, {
        polarity,
        required: false,
        role: "alternative-route",
      }),
      ...expressionRoles(expression.alternative_if.alternative, {
        polarity,
        required: false,
        role: "alternative-route",
      }),
  ...
  if (expression.veto_if) {
    return [
      ...expressionRoles(expression.veto_if.condition, { polarity, required, role }),
      ...expressionRoles(expression.veto_if.blocker, {
        polarity: polarity === "positive" ? "negative" : "positive",
        required,
        role: "blocker",
      }),
...
function displayStringErrors(option) {
  const values = [
    ["$.object.service_ref", option?.object?.service_ref],
    ...["jurisdictions", "geographies", "cohorts", "services"].flatMap((key) =>
      (option?.scope?.[key] || []).map((value, index) => [`$.scope.${key}[${index}]`, value])),
  ];
  const errors = [];
  for (const [path, value] of values) {
    if (
      ...
      || MARKUP_DISPLAY_CHARACTER.test(value)
      || IMPERATIVE_DISPLAY_LANGUAGE.test(value)
    ) {
      errors.push(problem(
        "UNSAFE_DISPLAY_STRING",
        path,
        "Display values must be trimmed single-line labels without controls, markup or imperative language.",
      ));
    }
  }
  return errors;
...
function conditionContractErrors(condition, asOf) {
  ...
  for (const [index, approval] of condition.governance.approved_by.entries()) {
    const approvedAt = strictUtcInstant(approval.approved_at);
    const path = `$.condition.governance.approved_by[${index}].approved_at`;
    if (approvedAt === null) {
      errors.push(problem(
        "INVALID_CONDITION_APPROVAL_AT",
        path,
        "Condition approval time must be an exact UTC instant ending in Z.",
      ));
      continue;
    }
    if (createdAt !== null && approvedAt < createdAt) {
      errors.push(problem(
        "CONDITION_APPROVAL_BEFORE_CREATION",
        path,
        "A condition cannot be approved before it is created.",
      ));
    }
    if (expiresAt !== null && approvedAt >= expiresAt) {
      errors.push(problem(
        "CONDITION_APPROVAL_AFTER_EXPIRY",
        path,
        "A condition approval must occur before condition expiry.",
      ));
    }
    if (asOfInstant !== null && approvedAt > asOfInstant) {
      errors.push(problem(
        "CONDITION_APPROVAL_AFTER_AS_OF",
        path,
        "A condition approval cannot occur after as_of.",
      ));
    }
  }
...
function conditionBindingErrors(option, condition) {
  ...
  if (!reachable.has(predicateRef)) {
    ...
    const roles = expressionRoles(condition.gates?.[option?.bound_gate])
      .filter(({ predicate_ref: ref }) => ref === predicateRef);
    if (
      roles.length === 0
      || roles.some(({ polarity, required, role }) =>
        polarity !== "positive" || required !== true || role !== "enabling")
    ) {
      errors.push(problem(
        "OUTCOME_NOT_POSITIVE_REQUIRED_ENABLER",
        "$.protected_outcome.condition_predicate_ref",
        "The protected outcome must be a positive, required enabling predicate for the selected gate.",
      ));
    }
  }
  ...
  if (predicate.operator === "neq") {
    errors.push(problem(
      ...
      "$.protected_outcome.operator",
      "neq has no unambiguous protected-outcome direction and is not supported.",
    ));
  }
...
function gateEligible(resolution, gate) {
  if (gate === "watch" || gate === "recover") {
    return resolution?.concurrent_duties_eligible?.[gate] === true;
  }
  if (gate === "graduate") return resolution?.exit_candidate_eligible === true;
  if (gate === "pause" || gate === "reverse") {
    return resolution?.safety_control === gate;
  }
  return resolution?.candidate_phase === gate && resolution?.candidate_phase_eligible === true;
}

export function selectedGateEvaluation(option, evaluationBundle) {
  const run = evaluationBundle.evaluation_run;
  const gate = option.bound_gate;
  return {
    path: `#/gates/${gate}`,
    evaluation_ref: {
      id: run.id,
      ...
      bundle_checksum: checksumJson(evaluationBundle),
      trust_state: "content-validated-unverified",
    },
    evaluated_at: run.evaluated_at,
    truth_state: run.gate_results[gate].state,
    eligibility_state: gateEligible(run.condition_resolution, gate) ? "eligible" : "ineligible",
    eligibility_basis: GATE_ELIGIBILITY_BASES[gate],
    authorisation_effect: "none",
  };
}

function evaluationBundleErrors(option, condition, evaluationBundle, asOf) {
  if (!evaluationBundle || typeof evaluationBundle !== "object" || Array.isArray(evaluationBundle)) {
    return [problem(
      "MISSING_EVALUATION_BUNDLE",
      "$.evaluation_bundle",
      "Compilation requires a completed evaluation bundle, not a caller gate assertion.",
    )];
  }

  const errors = [];
  const keys = Object.keys(evaluationBundle);
  if (
    keys.length !== 2
    || !Object.hasOwn(evaluationBundle, "observations")
    || !Object.hasOwn(evaluationBundle, "evaluation_run")
  ) {
    errors.push(problem(
      "EVALUATION_BUNDLE_SHAPE_INVALID",
      "$.evaluation_bundle",
      "The evaluation bundle must contain only observations and evaluation_run.",
    ));
  }

  const observations = evaluationBundle.observations;
  const run = evaluationBundle.evaluation_run;
  const observationsValid = Array.isArray(observations) && observations.every((observation, index) => {
    if (validateObservationSchema(observation)) return true;
    for (const error of validateObservationSchema.errors || []) {
      errors.push(problem(
        "EVALUATION_OBSERVATION_SCHEMA_INVALID",
        `$.evaluation_bundle.observations[${index}]${error.instancePath}`,
        error.message || "The evaluation observation is invalid.",
      ));
    }
    return false;
  });
  if (!Array.isArray(observations)) {
    errors.push(problem(
      "EVALUATION_OBSERVATIONS_INVALID",
      "$.evaluation_bundle.observations",
      "Evaluation observations must be an array.",
    ));
  }

  const runValid = validateEvaluationRunSchema(run);
  if (!runValid) {
    for (const error of validateEvaluationRunSchema.errors || []) {
      errors.push(problem(
        "EVALUATION_RUN_SCHEMA_INVALID",
        `$.evaluation_bundle.evaluation_run${error.instancePath}`,
        error.message || "The evaluation run must be a completed run.",
      ));
    }
  }

  const conditionValid = condition && typeof condition === "object" && validateConditionSchema(condition);
  if (conditionValid && observationsValid && runValid) {
    const bundleResult = validateEvaluationBundle(condition, observations, run);
    for (const error of bundleResult.errors) {
      errors.push(problem(
        `EVALUATION_BUNDLE_${error.code}`,
        `$.evaluation_bundle${error.path}`,
        error.message,
      ));
    }
  }

  if (!runValid) return errors;

  const evaluatedAt = strictUtcInstant(run.evaluated_at);
  const asOfInstant = strictUtcInstant(asOf);
  if (evaluatedAt === null) {
    errors.push(problem(
      "INVALID_GATE_EVALUATED_AT",
      "$.evaluation_bundle.evaluation_run.evaluated_at",
  ...
  if (run.gate_results?.[option?.bound_gate]?.state !== "true") {
    errors.push(problem(
      "GATE_NOT_TRUE",
      `$.evaluation_bundle.evaluation_run.gate_results.${option?.bound_gate}.state`,
      "The selected gate must deterministically evaluate to true.",
    ));
  }
  if (!gateEligible(run.condition_resolution, option?.bound_gate)) {
    errors.push(problem(
      "GATE_NOT_ELIGIBLE",
      "$.evaluation_bundle.evaluation_run.condition_resolution",
      "The selected gate must be eligible on its recomputed resolution axis.",
    ));
  }
  if (!isDeepStrictEqual(run.condition_definition, option?.condition_definition_ref)) {
    errors.push(problem(
      "GATE_CONDITION_MISMATCH",
      "$.evaluation_bundle.evaluation_run.condition_definition",
      "The completed evaluation must pin the same condition as the option.",
    ));
  }
  return errors;
}

function timeErrors(option, condition, evaluationBundle, asOf) {
  const errors = [];
  const asOfInstant = strictUtcInstant(asOf);
  if (asOf === undefined) {
    errors.push(problem(
  ...
  const conditionCreatedAt = strictUtcInstant(condition?.created_at);
  const conditionValidFrom = strictUtcInstant(condition?.governance?.valid_from);
  const evaluatedAt = strictUtcInstant(evaluationBundle?.evaluation_run?.evaluated_at);
  ...
  if (generatedAt !== null && conditionCreatedAt !== null && generatedAt < conditionCreatedAt) {
    errors.push(problem(
      "OPTION_GENERATED_BEFORE_CONDITION_CREATION",
      "$.provenance.generated_at",
      "The option cannot be generated before its condition is created.",
    ));
  }
  if (generatedAt !== null && conditionValidFrom !== null && generatedAt < conditionValidFrom) {
    errors.push(problem(
      "OPTION_GENERATED_BEFORE_CONDITION_VALID",
      "$.provenance.generated_at",
      "The option cannot be generated before its condition becomes valid.",
    ));
  }
  if (generatedAt !== null && evaluatedAt !== null && generatedAt < evaluatedAt) {
    errors.push(problem(
      "OPTION_GENERATED_BEFORE_EVALUATION",
      "$.provenance.generated_at",
      "The option cannot be generated before the completed evaluation it uses.",
    ));
  }
