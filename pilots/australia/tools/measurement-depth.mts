...
const previousOutput = 'pilots/australia/data/primary-care-depth-2026-09-10.json';
export const depthOutput = 'pilots/australia/data/primary-care-depth-2026-09-10.r2.json';
...
export function assessChange(earlier: any, latest: any, comparable: boolean): any {
  for (const point of [earlier, latest]) if (!point || point.unit !== 'percent' || !Number.isFinite(point.value) || point.value < 0 || point.value > 100) throw new Error('Change assessment requires finite percentage observations in [0,100]');
  ...
  if (widths.some(w => w == null)) return { difference_percentage_points: difference, approximate_significant: null, change_95ci: null, same_population_change_established: false, explanation: 'No retained sampling intervals: significance cannot be assessed. Administrative coverage changes are descriptive.' };
  ...
  return { difference_percentage_points: difference, change_95ci: [difference - halfWidth, difference + halfWidth].map(v => Number(v.toFixed(3))), approximate_significant: significant, same_population_change_established: comparable && significant, covariance_assumption: 'Zero covariance approximation; exact only for uncorrelated estimates. Source scope change is separate from sampling uncertainty.', explanation: significant ? `Change exceeds the approximate 95% interval around zero.${comparable ? '' : ' Survey scope changed, so a same-population change is not established.'}` : 'Change is within the approximate 95% interval around zero; a change beyond sampling variation is not established.' };
}
...
export function deriveMeasurementDepth(): any {
  ...
  const afterSeries = [
    ...
    return { id, condition_category: category, label, source_id: workbook.id, source_url: workbook.source_url, source_artifact_hash: workbook.sha256,
      population: 'Survey-scope persons aged 15 years and over who needed to see an after-hours GP in the preceding 12 months; Australia. The 2024-25 survey excludes very remote areas, Indigenous Community Strata and non-private dwellings, including nursing homes and homes for people with disabilities. Proxy interviews were permitted in defined circumstances; exclusion must be assessed for each specific item.',
      evidence_ceiling: 'All after-hours GP modalities, not urgent unsociable-hours telehealth alone. Main reasons do not identify all concurrent barriers. Random confidentiality adjustment prevents exact subtraction or reconciliation of components.',
      points: [...'BCDEFGHIJKLM'].map(col => ({ geography: 'Aust', period: cell('Table 7', `${col}6`).text.replace('–', '-'), value: Number(numericCell(cell('Table 7', `${col}${row}`).text).toFixed(1)), unit: 'percent', published_95ci_half_width: numericCell(cell('Table 7', `${col}${Number(row) + 15}`).text), numerator: null, source_cells: { table: 'Table 7', period: `${col}6`, value: `${col}${row}`, interval: `${col}${Number(row) + 15}`, definition: `A${row}`, footnote: 'A37' } })) };
  });
  ...
  const condition = (category: string, status: string, observation: string, sourceId: string, missing: string, why: string) => ({ condition_category: category, status, observation, source_id: sourceId, source_url: source(sourceId).source_url, source_artifact_hash: source(sourceId).sha256, source_refs: [{ source_id: sourceId, source_url: source(sourceId).source_url, source_artifact_hash: source(sourceId).sha256, label: source(sourceId).publisher }], missing_series: missing, why_missing: why });
  const items = [
    { id: 'atorvastatin-prescription', label: 'Clinically prescribed atorvastatin, dispensed by a community pharmacy', geography: 'NSW', conditions: [
      ...
      condition('permission', 'rule-context', 'AIHW describes PBS eligibility through a current Medicare card or reciprocal arrangements. Dispensing permission and subsidy eligibility are separate questions; PBS eligibility alone does not establish permission to dispense privately.', 'aihw-medicines', 'NSW atorvastatin needs matched to a valid prescription and lawful dispensing pathway (PBS or private), with PBS item restrictions, subsidy entitlement and approval or refusal recorded separately on the attempted dispensing date.', 'Retained AIHW overview describes prescribing and subsidy categories; it publishes no person-by-prescription dispensing approval denominator.'),
  ...
  const prescriptionPrice = items[0].conditions.find(c => c.condition_category === 'price');
  for (const [id, label] of [['pc-primary-care-dataset', 'NSW prescription cost-delay: RoGS 10A.33'], ['pbs-safety-net', 'PBS 2026 safety-net thresholds']]) prescriptionPrice.source_refs.push({ source_id: id, source_url: source(id).source_url, source_artifact_hash: source(id).sha256, label });
  for (const c of items[1].conditions.filter(c => ['price', 'availability'].includes(c.condition_category))) c.source_refs.push({ source_id: 'abs-methodology', source_url: source('abs-methodology').source_url, source_artifact_hash: source('abs-methodology').sha256, label: 'ABS survey scope and proxy interview rules' });
  return { id: 'australia-primary-care-depth-2026-09-10.r2', as_of: '2026-09-10', correction: { supersedes_path: previousOutput, supersedes_sha256: digest(read(previousOutput)), reason: 'Review: add claim-specific source references, disclose institutional/Indigenous survey scope, separate subsidy from dispensing permission and make interval assessment direction-neutral. Original measurements unchanged.', source_values_changed: false }, provenance: 'commissioned-proposal', historical_inputs_preserved: true, small_base_threshold: SMALL_BASE_THRESHOLD, source_reviews: [...sources, ...reviewed.source_reviews], policy, after_hours_series: afterSeries, histories, items, significance_method: { source_url: source('abs-methodology').source_url, source_hash: source('abs-methodology').sha256, section: 'Calculating differences; Significance testing', formula: 'SE=95%MOE/1.96; approximate SE difference=sqrt(SE1^2+SE2^2); two-sided 95% interval=difference +/-1.96*SE difference', exact_covariance_available: false, multiple_comparison_adjustment: false }, workbook_footnotes: Object.fromEntries(Object.entries(tables).map(([name, cells]: [string, any]) => [name, cells.filter((c: any) => /^A/.test(c.address) && c.text.length > 80).map((c: any) => ({ address: c.address, text_sha256: digest(c.text), character_count: c.text.length }))])) };
}
