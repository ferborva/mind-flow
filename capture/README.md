# capture

Near-verbatim records of conversations. The raw material for everything else.

One file per conversation: `YYYY-MM-DD-slug.md`.

**Fidelity over polish.** Filler words go, sentences get closed, obvious
transcription noise gets cleaned. Nothing else. Phrasing stays his, metaphors
stay his, tangents stay in — the tangents are frequently where the actual
insight is hiding. If something was said half-formed, it stays half-formed and
gets marked, because half-formed is where the real thinking shows.

Captures are append-only in spirit. They are never deleted and never rewritten
to read better. If a later conversation contradicts an earlier one, both stand;
the contradiction is itself information.

Template: `meta/templates/capture.md`
