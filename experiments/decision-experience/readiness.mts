...
export function renderMaterial(pack: RenderPack, kind: 'conventional' | 'station'): string {
  ...
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Decision rehearsal: ${kind}</title><style>body{font:18px/1.55 system-ui;margin:auto;max-width:850px;padding:24px;overflow-wrap:anywhere}*{box-sizing:border-box}summary{cursor:pointer}summary:focus-visible{outline:3px solid #ad6516}h1{line-height:1.2}article{margin:16px 0}${css}</style></head><body>
