...
const reviewBoundary = 'Materials review only. No responses are collected. Recruitment blocked. Assignment is not implemented. Not a crisis warning or personal advice.';
...
function styles(layout: Layout | 'reviewer') {
  ...
  if (layout !== 'station') return common + 'body{background:#fff;color:#192c2c}header{max-width:900px}.material-table{margin:1em 0}.review-answer{padding:1em;border:1px solid #96aaa5;margin:1em 0}@media(max-width:540px){.material-table tbody,.material-table tr,.material-table th,.material-table td{display:block;width:100%}.material-table tr{border-bottom:1px solid #a1afad;padding:.45em 0}.material-table th,.material-table td{border:0;padding:.35em .25em}}';
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    console.log('Static reasoning-slice edition reproduces: 14 material items, three tasks, two layouts. Human gates remain blocked.');
