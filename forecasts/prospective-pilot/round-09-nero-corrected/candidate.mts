...
import { root, recordedAt, baselinePath, source, sourceId, sourceUrl, sha256, jsonBytes } from "../round-09-nero/basis.mts";
...
import { matureForecastContractIdentity } from "../issuance-binding/round-09-corrected-validate.mjs";
import { runNeroBaseline } from "../issuance-binding/baseline-execution.mjs";

import { NERO_TARGET, neroTargetProse, assertNeroTargetConsistency } from "./target-policy.mjs";
export const directory = "forecasts/prospective-pilot/round-09-nero-corrected";
const basisDirectory = "forecasts/prospective-pilot/round-09-nero";
const sourceObservedAt = "2026-09-10T00:28:43Z";
...
const parameters = { occupation_code: NERO_TARGET.occupation_code, sa4_code: NERO_TARGET.sa4_code, first_month: "2024-08", last_month: "2026-08", horizon_months: 2 };
...
export function prepareNeroCandidate({ sealAt, issueOpensAt, issuedAt, sourceCommit, externalReceipt = null }: CandidateClocks) {
  if (!(Date.parse(sealAt) >= Date.parse(sourceObservedAt) && Date.parse(sealAt) < Date.parse(issueOpensAt) &&
  ...
  const campaignId = "campaign.nero.5311.102.october-2026.correction-1";
  const protocolId = "protocol.nero.5311.102.october-2026.correction-1.v1";
  const targetId = "target.nero.5311.102.october-2026.correction-1";
  ...
  const forecast: any = { schema_version: "1.4.0", id: "forecast.nero.5311.102.october-2026.correction-1.v1",
  ...
  Object.assign(protocol.campaign.manifest, { manifest_id: "manifest.nero.5311.102.october-2026.correction-1.v1", campaign_id: campaignId,
  ...
  const resolverParameters = objectArtifact({ occupation_code: NERO_TARGET.occupation_code, sa4_code: NERO_TARGET.sa4_code, date: NERO_TARGET.date, count_baseline: NERO_TARGET.count_baseline,
    dependencies: Object.fromEntries([`${basisDirectory}/basis.mts`, `${basisDirectory}/resolver.mts`, "dashboard/tools/build-nero-baseline.mjs",
      "forecasts/prospective-pilot/tests/round-09-nero-corrected.test.mjs", `${directory}/target-policy.mjs`, `${directory}/candidate.mts`,
  ...
  assertNeroTargetConsistency(forecast, protocol, resolverParameters.document, [parameters, parameters]);
