...
import { root, recordedAt, baselinePath, source, sourceId, sourceUrl, sha256, jsonBytes } from "./basis.mts";
...
import { matureForecastContractIdentity } from "../issuance-binding/round-10-country-validate.mjs";
import { runCountryBaseline, runCanadaSeasonalForecaster } from "../issuance-binding/country-baseline-execution.mjs";
import { retainedCanadaAbsence, CANADA_ABSENCE_PATH } from './absence.mts';
import { calendarPath, retainedCanadaCalendar } from './draft.mts';

import { CANADA_TARGET, canadaTargetProse, assertCanadaTargetConsistency } from "./target-policy.mjs";
export const directory = "forecasts/prospective-pilot/round-10-canada";
const basisDirectory = "forecasts/prospective-pilot/round-10-canada";
type CandidateClocks = { sealAt: string; issueOpensAt: string; issuedAt: string; sourceCommit: string; externalReceipt?: any; sourceAbsencePath?: string };
...
export function prepareCanadaCandidate({ sealAt, issueOpensAt, issuedAt, sourceCommit, externalReceipt = null, sourceAbsencePath=CANADA_ABSENCE_PATH }: CandidateClocks) {
  const retainedAbsence=retainedCanadaAbsence(sourceAbsencePath),sourceObservedAt=retainedAbsence.observedAt;
  retainedCanadaCalendar();
  if (!(Date.parse(sealAt) >= Date.parse(recordedAt) && Date.parse(sealAt) >= Date.parse(sourceObservedAt) && Date.parse(sealAt) < Date.parse(issueOpensAt) &&
  ...
  const sourceKernel = source(`${basisDirectory}/kernel.json`);
  const sourceSignalRegistry = source(`${basisDirectory}/registry.json`);
  ...
  const campaignId = "campaign.canada.unemployment.october-2026";
  const protocolId = "protocol.canada.unemployment.october-2026.v1";
  const targetId = "target.canada.unemployment.october-2026";
  const baselineImplementation = artifact("forecasts/prospective-pilot/issuance-binding/country-baseline-execution.mjs");
  const baselineConformance = artifact("forecasts/prospective-pilot/issuance-binding/country-baseline-conformance.json");
  ...
  for (const role of ["reference", "naive"]) {
    ...
    const algorithmId = role === "reference" ? "mind-flow.country-two-month-direction" : "mind-flow.equal-probability";
    const baselineId = `family.canada.${role}.v1`;
    ...
    baselines[role] = { campaign_id: campaignId, family_id: baselineId,
      ...
      probability: runCountryBaseline(algorithmId, inputSource.document.history, parameters),
      method: role === "reference" ? "Count nonnegative two-month changes from September 2016 through August 2026 in the retained August 2026 vintage; (successes+1)/(comparisons+2)." : "Return 0.5 after the same input eligibility checks.",
  ...
  const prose = canadaTargetProse();
  const forecast: any = { schema_version: "1.4.0", id: "forecast.canada.unemployment.october-2026.v1",
    epistemic_class: "forecast", forecast_use: "research_only", status: "issued",
    title: prose.title,
    question: prose.question,
    issued_at: issuedAt, resolve_after: "2026-11-06T00:00:00Z", resolve_by: "2026-12-31T23:59:59Z",
    probability: runCanadaSeasonalForecaster(inputSource.document.history).probability,
    target: { event: prose.event,
      unit: "binary event on a reported unemployment percentage", scope: targetScope, signal_id: predicate.signal_ref.signal_id,
      ...
      resolution_rule: prose.resolution_rule,
      resolution_source: sourceUrl, resolution_event_id: "event.canada.unemployment.2026-10", independence_cluster_id: "cluster.ilo-canada-lfs.2026",
      resolver: { resolver_id: "mind-flow.binary-threshold-json", resolver_version: "1.1.0",
        measure: metric.measure, observation_unit: metric.unit, operator: "gte", threshold: 0.07302 },
      observation_window_start: "2026-10-01T00:00:00Z", observation_window_end: "2026-10-31T23:59:59Z",
      outcome_publication_not_before: "2026-11-06T00:00:00Z" },
    baseline: baselines.reference, naive_baseline: baselines.naive,
    method: { kind: "model", description: "Use all nine August-to-October pairs in the fixed baseline window with Laplace smoothing, 1/11. Method chosen after inspecting historical seasonality; retain all-month 0.483333 and naive 0.5 comparators. Nine potentially dependent pairs do not establish calibration, skill or validated binomial uncertainty.", version: "1.0.0" },
    data_vintages: [{ source: `${baseUrl}/blob/${sourceCommit}/${baselinePath}`, retrieved_at: recordedAt,
      vintage: "September 2026 retained ILO Canadian reported monthly vintage", checksum: inputSource.sha256 },
    { source: inputSource.document.source.url, retrieved_at: "2026-09-10T06:54:55Z", vintage: "2026-08", checksum: inputSource.document.source.body_sha256 }],
    provenance: { author: "Ren", class: "agent-proposal", issued_commit: sourceCommit,
      disclaimer: "Research-only agent forecast of reported unemployment. Availability is a commissioned research relationship, not a binding category diagnosis. No calibration, causal, agency, access or action claim. issued_commit identifies the source checkout at issuance; the containing record commit is discoverable from Git history and cannot be embedded in itself." },
    assumptions: ["The selected source-native code and label pair remains comparable across the frozen August and first October releases.",
      "Historical two-month signs in one reported and revised vintage provide a mechanical baseline, not independent trials or calibrated future probabilities.",
      "First retained release may differ from later corrections; all later changes remain separately visible.",
      "The fraction is of the Canadian labour force aged 15 and older, not total population or income-route losses."],
    counter_hypotheses: ["The October reported unemployment percentage is below 7.302.", "Sampling, revisions or classification changes break historical comparability.",
      "The October ILO cell is unavailable by resolution close."],
  ...
  Object.assign(protocol.campaign.manifest, { manifest_id: "manifest.canada.unemployment.october-2026.v1", campaign_id: campaignId,
  ...
  const resolverParameters = objectArtifact({ ...CANADA_TARGET,
    dependencies: Object.fromEntries([ `${directory}/basis.mts`, `${directory}/resolver.mts`, `${directory}/target-policy.mjs`, `${directory}/candidate.mts`,
      ...
      "forecasts/prospective-pilot/operational-clock.mjs", "forecasts/prospective-pilot/tests/operational-clock.test.mjs",
      "forecasts/prospective-pilot/issuance-binding/round-10-country-validate.mjs"].map((path) => [path, artifact(path).sha256])) });
  ...
  const absence: any = { sequence: 1, source_id: sourceId, state: "reported_absent", observed_at: sourceObservedAt,
  ...
  assertCanadaTargetConsistency(forecast, protocol, resolverParameters.document, [parameters, parameters]);
