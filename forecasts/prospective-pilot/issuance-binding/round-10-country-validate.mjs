// Corrected Round 09 edition. Prior editions remain immutable historical bytes.
// This adds native/prose consistency and seals the corrected preparation dependencies.
...
import { runCountryBaseline } from "./country-baseline-execution.mjs";
import { deriveCanadaDraft, calendarPath, retainedCanadaCalendar } from "../round-10-canada/draft.mts";
import { CANADA_ABSENCE_PATH, retainedCanadaAbsence } from '../round-10-canada/absence.mts';
import { buildCanadaBasis, jsonBytes } from '../round-10-canada/basis.mts';
...
const baselineImplementationBytes = readFileSync(new URL("./country-baseline-execution.mjs", import.meta.url));
const baselineConformanceBytes = readFileSync(new URL("./country-baseline-conformance.json", import.meta.url));
...
function assessResolverArtifacts(protocol, forecast, sources, errors) {
  ...
  const allowed = new Set(["forecasts/prospective-pilot/round-10-canada/basis.mts",
    "forecasts/prospective-pilot/round-10-canada/resolver.mts",
    ...
    "forecasts/prospective-pilot/issuance-binding/round-10-country-validate.mjs"]);
  if (!parameters?.dependencies || typeof parameters.dependencies !== "object" || Array.isArray(parameters.dependencies)) {
    ...
    if (!isDeepStrictEqual(Object.keys(parameters.dependencies).sort(), [...allowed].sort())) {
      errors.push(issue("RESOLVER_ARTIFACT_PARAMETERS_INVALID", "/matureForecastSources/resolverArtifacts/parameters", "Canadian adapter requires every reviewed native extraction dependency"));
    }
  }
...
function reproduceBaseline(protocol, forecast, sources, role, errors) {
  ...
  if (!["mind-flow.country-two-month-direction", "mind-flow.equal-probability"].includes(registered?.algorithm_id)) return false;
  try {
    ...
    if (manifest.input_checksums.length !== 1 || artifacts.inputs?.length !== 1 ||
      ...
      throw new Error("one exact retained Canadian input must reproduce the sealed manifest digest");
    }
    ...
    for (const [algorithm, expected] of Object.entries(vectors.expected)) {
      if (runCountryBaseline(algorithm, vectors.input, vectors.parameters) !== expected) {
    ...
    if (!isDeepStrictEqual(input, deriveCanadaDraft())) throw new Error("Canadian baseline input must exactly replay the pinned native source");
    const computed = runCountryBaseline(registered.algorithm_id, input.history, parameters);
...
export function assessFutureIssuanceBinding({
  ...
  try {
    const parameters = (artifact) => JSON.parse(Buffer.from(artifact?.bytes || []).toString("utf8"));
    assertCanadaTargetConsistency(parsedForecast.document, parsedProtocol.document,
      parameters(matureForecastSources?.resolverArtifacts?.parameters),
      ["reference", "naive"].map((role) => parameters(matureForecastSources?.[`${role}BaselineArtifacts`]?.parameters)));
    retainedCanadaCalendar();
    const basis=buildCanadaBasis();
    if(!Buffer.from(matureForecastSources?.sourceKernel?.bytes??[]).equals(jsonBytes(basis.kernel))||!Buffer.from(matureForecastSources?.sourceSignalRegistry?.bytes??[]).equals(jsonBytes(basis.registry)))throw new Error('Canadian issue basis must exactly replay the literal statistical predicate and correlated-only native source relationship');
    const absence=retainedCanadaAbsence(),event=parsedProtocol.document?.source_chronology?.events?.[0];
    if(event?.state!=='reported_absent'||event?.observed_at!==absence.observedAt||event?.artifact_sha256!==null)throw new Error('source chronology must bind the exact fresh retained absence capture; outcome bytes remain absent');
  } catch (error) {
    errors.push(issue("CANADA_NATIVE_TARGET_PROSE_CONFLICT", "/target", error.message));
  }
