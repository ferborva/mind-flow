...
import { extractCanadaCompanion, runCountryBaseline, CANADA_SELECTOR, CANADA_LABELS } from '../country-baseline-execution.mjs';
...
test('extractor preserves native values and rejects target contamination, note changes and duplicates',()=>{
  const row={...CANADA_SELECTOR,...CANADA_LABELS,time:'2026M08',obs_value:'7.302',obs_status:'',note_classif:'',note_indicator:'I12:422',note_source:'R1:3513'};
  const header=Object.keys(row);const csv=rows=>[header.map(x=>JSON.stringify(x)).join(','),...rows.map(r=>header.map(k=>JSON.stringify(r[k])).join(','))].join('\n');
  ...
  assert.throws(()=>extractCanadaCompanion(csv([{...row,'source.label':'Seasonally adjusted headline'}])),/label/);
  assert.throws(()=>extractCanadaCompanion(csv([{...row,'obs_status.label':'Estimated'}])),/label/);
  assert.throws(()=>extractCanadaCompanion(csv([row]).replace('\n',',"classif2"\n')+',"unknown"'),/dimension/);
});
