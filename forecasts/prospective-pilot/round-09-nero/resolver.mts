...
export function neroOutcomePayload(rows: string[][], forecast: any) {
  ...
  if (target.observation_window_start !== "2026-10-01T00:00:00Z" ||
      ...
      target.signal_id !== "signal.nero.5311.102.bounded-stock" ||
      target.condition_id !== "condition.nero.5311.102.stock") {
    throw new Error("NERO adapter only resolves the frozen October 2026 stock target");
  }
  const selected = rows.filter((row) => row[2] === "102" && row[4] === "5311" && row[6] === "2026-10-15");
  if (selected.length !== 1) throw new Error("exactly one October General Clerks Central Coast cell is required");
  const row = selected[0];
  if (row.length !== 8 || row[1] !== "NSW" || row[3] !== "Central Coast" || row[5] !== "General Clerks" ||
...
export async function extractNeroResolution(archivePath: string, forecast: any) {
  ...
  for await (const row of await rowsFromZip(archivePath)) {
    if (row[2] === "102" && row[4] === "5311" && row[6] === "2026-10-15") selected.push(row);
  }
