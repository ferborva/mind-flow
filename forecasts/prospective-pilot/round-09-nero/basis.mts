...
export const directory = "forecasts/prospective-pilot/round-09-nero";
export const recordedAt = "2026-09-10T00:00:41Z";
...
export function boundedEmploymentIndex(value: number): number {
  ...
  return value / (value + 3092);
}

export function buildNeroBasis() {
  const baseline = source(baselinePath);
  const selected = baseline.document.series.filter((item: any) => item.occupation_code === "5311" && item.sa4_code === "102");
  if (selected.length !== 1 || selected[0].latest.value !== 3092 || selected[0].latest.date !== "2026-08-15") {
    throw new Error("The frozen August 2026 Central Coast General Clerks baseline changed");
  }
  const signal: any = {
    signal_id: "signal.nero.5311.102.bounded-stock", definition_version: "1.0.0",
    label: "Bounded NERO employment-stock index, General Clerks, Central Coast",
    construct: "Modelled employment stock x transformed to x/(x+3092); exposure context, not vacancies or worker opportunity",
    population: "NERO occupation code 5311, General Clerks, resident in SA4 code 102, Central Coast",
    estimand: "NERO modelled employed-person count x divided by x plus the frozen August 2026 count 3092",
  ...
  const scope = { jurisdictions: ["Australia"], geographies: ["SA4 102: Central Coast"],
    cohorts: ["ANZSCO4 5311: General Clerks"], services: ["Modelled employment stock, exposure context only"] };
  const definition: any = {
    condition_id: "condition.nero.5311.102.stock", definition_version: "1.0.0", condition_category: "availability",
    proposition: "The modelled employment stock is at least the frozen August 2026 count 3092; this is exposure context only.",
    claim: { who: signal.population, verb: "have", object: "modelled employment stock at or above 3092",
      standard: "bounded employment index x/(x+3092) is at least 0.5", polarity: "affirmative",
  ...
  const event: any = { sequence: 1, event_id: "event.nero.5311.102.stock.added", operation: "added", recorded_at: recordedAt,
  ...
  const observation: any = { observation_id: "observation.nero.5311.102.2026-08", classification: "measured-observation",
  ...
  const evidenceEvent: any = { sequence: 1, evidence_event_id: "evidence-event.nero.5311.102.2026-08.added",
  ...
  const kernel: any = { schema_version: "1.1.0", kernel_id: "kernel.nero.5311.102.prospective",
  ...
  const measurementScope = { construct_id: "construct.nero.5311.102.stock", population: signal.population,
    geography: "SA4 102: Central Coast, Australia", statistical_unit: "one modelled occupation-SA4 series",
    period, unit: signal.unit, denominator: "x plus frozen August 2026 count 3092; not a population denominator", aggregation_level: signal.aggregation };
  const item: any = { signal_id: signal.signal_id, label: signal.label, status: "shadow",
    ...
    value_of_information: { decision_ref: null, learning_value: "Test whether a prospective modelled-stock claim can be resolved faithfully.",
      next_observation: "First retained October 2026 NERO release for occupation 5311 and SA4 102." },
  ...
  item.metric_contract = { metric_id: "metric.nero.5311.102.bounded-stock", measure: signal.estimand,
    ...
    evaluation_rule: "Transform the exact series count x to x/(x+3092), then compare gte 0.5. No population-share interpretation." };
  ...
  const registry = { schema_version: "1.1.0", registry_id: "registry.nero.5311.102.prospective",
