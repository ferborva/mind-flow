...
import { matureForecastContractIdentity } from "../issuance-binding/validate.mjs";
...
const parameters = { occupation_code: "5311", sa4_code: "101", first_month: "2024-08", last_month: "2026-08", horizon_months: 2 };
...
export function prepareNeroCandidate({ sealAt, issueOpensAt, issuedAt, sourceCommit, externalReceipt = null }: CandidateClocks) {
  ...
  const campaignId = "campaign.nero.5311.101.october-2026";
  const protocolId = "protocol.nero.5311.101.october-2026.v1";
  const targetId = "target.nero.5311.101.october-2026";
  ...
  const forecast: any = { schema_version: "1.4.0", id: "forecast.nero.5311.101.october-2026.v1",
    epistemic_class: "forecast", forecast_use: "research_only", status: "issued",
    title: "October NERO General Clerks employment stock in Capital Region versus the frozen August count",
    question: "Will the first retained October 2026 NERO release report at least 4217 employed persons for occupation 5311, General Clerks, in SA4 101, Capital Region?",
    ...
    target: { event: "The first retained October 2026 NERO value for General Clerks in Capital Region is at least 4217.",
      ...
      resolution_rule: "Retain the first October archive and first-presence receipt. Select exactly one CSV cell with anzsco4_code 5311, sa4_code 101 and date 2026-10-15. Derive x/(x+4217). Resolve 1 iff this is at least 0.5 (equivalently x>=4217), else 0. No substitutions or later-vintage shopping; apply the prewritten resolution and void procedure.",
      resolution_source: sourceUrl, resolution_event_id: "event.nero.5311.101.2026-10", independence_cluster_id: "cluster.nero-model.2026",
    ...
    counter_hypotheses: ["The October modelled count is below 4217.", "Smoothing, revisions or classification changes break historical comparability.",
  ...
  Object.assign(protocol.campaign.manifest, { manifest_id: "manifest.nero.5311.101.october-2026.v1", campaign_id: campaignId,
  ...
  const absence: any = { sequence: 1, source_id: sourceId, state: "reported_absent", observed_at: "2026-09-09T10:44:49Z",
