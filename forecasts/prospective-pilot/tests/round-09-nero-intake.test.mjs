...
import { checkNeroFirstPresence, inspectNeroRows, requireNeroResolutionAdmission, prepareNeroResolutionEvidence, admitNeroResolution } from "../round-09-nero/resolution-intake.mjs";
import { evaluateForecastCohort } from "../round-09-nero/current-evaluation.mjs";
import { singleMemberZip } from "./support/nero-zip-fixture.mjs";

const json = (name) => JSON.parse(readFileSync(new URL(`../round-09-nero/${name}`, import.meta.url)));
...
test("first-presence intake requires the native October filename and exact anchored receipt", () => {
  ...
  for (const change of [
    ...
    { publishedAt: "2026-10-31T23:59:59Z" },
    { retrievedAt: "2026-12-07T00:00:01Z" },
...
test("a first presence before the frozen publication cutoff cannot become prospective evidence", () => {
  const input = context();
  const first = input.chronologyEvents[1];
  first.observed_at = "2026-10-31T23:59:59Z";
  first.event_sha256 = sourceChronologyEventSha256(first);
  ...
  input.publishedAt = first.observed_at;
  input.retrievedAt = first.observed_at;
  assert.throws(() => checkNeroFirstPresence(input), /chronology|clocks/);
});
